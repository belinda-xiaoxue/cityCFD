/******************************************************************/
/****CITY SCALE CFD£¨CSCFD£©Version 3: 3D (both meso and micro)****/
...
#define Por_xe 1000.0  			/* porous region 1 ends   in x-direction*/
/* Turbulence ralated */
   ...
   /* Loop over all cell threads in the domain */
	thread_loop_c (t,domain)
	{
		begin_c_loop (c,t)
		{
			...
			if( z <= H_Urb)
			{
				if (x_s>=Por_xs && x_s<=Por_xe && y_s>=Por_xs && y_s<=Por_xe) 
				{
					tmp_por = Por_0;
...
/***set the city area as porous medium and set 1 for clear region****/
...
DEFINE_SOURCE(kinetic_source,c,t,dS,eqn) /*k*/
{
	...
	con_stra = C3*g_acc*C_MU_T(c,t)*C2/0.85;
	tmp_K=C_UDMI(c,t,1);
	tmp_C2=C_UDMI(c,t,2);;
	Q=sqrt(u*u+v*v+w*w);
	Fk_1=u*u*dudx+v*v*dvdy+w*w*dwdz+u*v*(dudy+dvdx)+u*w*(dwdx+dudz)+v*w*(dwdy+dvdz);
	mu_t1 = rho*CMU*k/d;
	Fk=mu_t1*Fk_1/Q;
...
/****		TDR   source				****/
DEFINE_SOURCE(disspation_source,c,t,dS,eqn) /*epsilon*/
{
	real source;
	real tmp_K, tmp_C2;
	...
	con_stra = Ce1*tanh(C_W(c,t)/C_U(c,t))*C3*g_acc*C_MU_T(c,t)*C2/(0.85*C_K(c,t));
	tmp_K=C_UDMI(c,t,1);
	tmp_C2=C_UDMI(c,t,2);;
	Q=sqrt(u*u+v*v+w*w);
	con1=-2*Por_0*mu_l*tmp_K;
